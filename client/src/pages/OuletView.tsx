...
import ExpenseTrend from '../components/expenses/ExpenseTrend';
...
const OutletView: React.FC = () => {
  ...
  const [shopExpenseRefresh, setShopExpenseRefresh] = useState(0);
  ...
  const handleExpenseSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        // Trigger refresh of expense trend
        setShopExpenseRefresh(prev => prev + 1);
      }
    } catch (error: any) {
      setMessage({
          ...
          <div className="space-y-6">
            ...
            {shop?.id || shop?._id ? (
              <ExpenseTrend
                ...
                filterParam={{ shopId: Number(shop.id || shop._id) }}
                title={`Shop Expenses - ${shop?.shopName || shop?.name || ''}`}
                showShopColumn={false}
                refreshTrigger={shopExpenseRefresh}
              />
            ) : (
              <p className="text-center text-gray-500 py-8">Shop data not available</p>
            )}
          </div>
        );
