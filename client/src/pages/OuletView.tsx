...
const OutletView: React.FC = () => {
  ...
  const [shopExpenseData, setShopExpenseData] = useState<any | null>(null);
  ...
  useEffect(() => {
    if (!shop) return;
    const shopId = Number(shop.id || shop._id);
    if (!shopId) return;
    ...
    const fetchShopExpenses = async () => {
      ...
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_SERVER_HEAD}/api/expenses/?page=${expensePage}&limit=${expenseLimit}&shopId=${shopId}`,
          { withCredentials: true },
        );
        setShopExpenseData(response.data.data);
      } catch (error) {
        console.error('Failed to fetch shop expenses:', error);
      } finally {
    ...
    fetchShopExpenses();
  }, [shop, expensePage, expenseLimit]);

  // Expense data processing helpers
  const shopExpenseTrendData = useMemo(() => {
    if (!shopExpenseData?.expenses) return [];
    return shopExpenseData.expenses
      .slice()
  ...
  const shopExpenseCategoryBreakdown = useMemo(() => {
    if (!shopExpenseData?.expenses) return [];
    const breakdown = shopExpenseData.expenses.reduce((acc: any, exp: any) => {
  ...
  const shopExpenseStatusCount = useMemo(() => {
    if (!shopExpenseData?.expenses) return {};
    return shopExpenseData.expenses.reduce((acc: any, exp: any) => {
  ...
  const expensePieColors = ['#0088FE', '#FF8042', '#00C49F', '#FFBB28', '#8884d8', '#82ca9d', '#FF6B6B'];
  ...
  const handleExpenseSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        const shopId = Number(shop.id || shop._id);
        const response = await axios.get(
          `${import.meta.env.VITE_SERVER_HEAD}/api/expenses/?page=1&limit=${expenseLimit}&shopId=${shopId}`,
          { withCredentials: true },
        );
        setShopExpenseData(response.data.data);
      }
          ...
          <div className="space-y-6">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-primary">
                      Ksh {shopExpenseData?.totalAmount ? Number(shopExpenseData.totalAmount).toLocaleString() : '0'}
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-green-600">
                      Ksh {shopExpenseData?.averageAmount ? Number(shopExpenseData.averageAmount).toLocaleString() : '0'}
                    </p>
                  </div>
              ...
              <div className="rounded-lg bg-bodydark1 dark:bg-boxdark dark:text-whiten shadow px-4 py-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm">
                      <span className="text-green-600">{shopExpenseStatusCount['APPROVED'] || 0}</span> /{' '}
                      <span className="text-red-600">{shopExpenseStatusCount['REJECTED'] || 0}</span> /{' '}
                      <span className="text-yellow-600">{shopExpenseStatusCount['PENDING'] || 0}</span>
                    </p>
                  </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="rounded-lg bg-white dark:bg-boxdark shadow-md p-6">
                <h3 className="text-lg font-semibold mb-4 text-gray-800 dark:text-white">Expense Over Time</h3>
                {expenseLoading ? (
                  <div className="flex items-center justify-center h-60">
                    <div className="h-12 w-12 animate-spin rounded-full border-b-2 border-t-2 border-blue-500"></div>
                  </div>
                ) : shopExpenseTrendData.length > 0 ? (
                  <ResponsiveContainer width="100%" height={240}>
                    <LineChart data={shopExpenseTrendData}>
              ...
              <div className="rounded-lg bg-white dark:bg-boxdark shadow-md p-6">
                ...
                ) : shopExpenseCategoryBreakdown.length > 0 ? (
                  <ResponsiveContainer width="100%" height={240}>
                    <PieChart>
                      <Pie
                        data={shopExpenseCategoryBreakdown}
                        dataKey="value"
                        nameKey="name"
                        cx="50%"
                        cy="50%"
                        ...
                        label
                      >
                        {shopExpenseCategoryBreakdown.map((_: any, index: number) => (
            ...
            <div className="rounded-lg bg-white dark:bg-boxdark shadow-md p-6">
                ...
                {shopExpenseData && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm dark:text-bodydark">
                      Page {expensePage} of {shopExpenseData.totalPages}
                    </span>
                    <button
                      onClick={() => setExpensePage(prev => Math.min(shopExpenseData.totalPages, prev + 1))}
                      disabled={expensePage === shopExpenseData.totalPages || expenseLoading}
                      className="px-3 py-1 rounded bg-gray-200 dark:bg-meta-4 disabled:opacity-50 hover:bg-gray-300 dark:hover:bg-gray-600"
                    >
                      Next
                    </button>
              ...
              {expenseLoading ? (
              ...
              ) : shopExpenseData?.expenses && shopExpenseData.expenses.length > 0 ? (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm text-left">
                    ...
                    <tbody>
                      {shopExpenseData.expenses.map((expense: any) => (
                        <tr key={expense.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                          ...
                          <td className="px-6 py-4">
                            <span className={`px-2 py-1 rounded text-xs font-medium ${getExpenseStatusColor(expense.status)}`}>
                              {expense.status}
                            </span>
                ...
                <p className="text-center text-gray-500 py-8">No expense data available for this outlet</p>
              )}
            </div>
          </div>
        );
