...
const Receipt: React.FC<ReceiptProps> = ({ saleResponse, onClose }) => {
  ...
  // Calculate totals — exclude consignment items (financeStatus === 'pending') from displayed total
  const regularSales = saleResponse.filter((sale) => sale.financeStatus !== 'pending');
  const consignmentSales = saleResponse.filter((sale) => sale.financeStatus === 'pending');

  // Pure consignment sale = ALL items are consignment
  const isPureConsignment = consignmentSales.length > 0 && regularSales.length === 0;

  const total = regularSales.reduce(
        ...
        <div id="printable-receipt" className="font-sans bg-white p-3 md:p-6 text-gray-800 leading-relaxed w-full">
          <div className="max-w-7xl mx-auto">
            <div className="grid grid-cols-2 gap-2 md:gap-4 items-start border-b border-gray-200 pb-4 mb-4">
              ...
              <div className="text-right">
                <h1 className="text-sm sm:text-base md:text-xl font-black bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
                  {isPureConsignment ? 'CONSIGNMENT RECEIPT' : 'SALES RECEIPT'}
                </h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                ...
                <div className="pb-4 border-b border-dashed border-gray-200">
                  ...
                  <div className="border-t border-b border-gray-200 py-3 space-y-2">
                    {saleResponse.map((sale) => {
                      const isConsignmentSale = sale.financeStatus === 'pending';
                      return (
                        <div key={sale.id} className="bg-gray-50 p-3 rounded-lg mb-2 last:mb-0">
                          ...
                          {isConsignmentSale ? (
                            <div className="flex items-center justify-between mt-2">
                              <span className="bg-purple-100 text-purple-800 px-2 py-0.5 rounded text-xs font-semibold uppercase">Consignment</span>
                              <span className="text-sm text-gray-400 italic">Financed sale</span>
                            </div>
                          ) : (
              ...
              {/* Right Column */}
              <div className="space-y-4">
                {/* Payment Details — hidden for pure consignment */}
                {!isPureConsignment ? (
                  <div className="pb-4 border-b border-dashed border-gray-200 text-sm">
                    ...
                    <div className="flex justify-between font-bold text-lg border-t border-dashed border-gray-300 pt-3 mt-3">
                      <span>{consignmentSales.length > 0 ? 'Cash Total:' : 'Total Amount:'}</span>
                      <span>
                        {new Intl.NumberFormat('en-KE', {
                          style: 'currency',
                          currency: 'KES',
                    ...
                    {consignmentSales.length > 0 && (
                      <p className="text-xs text-gray-400 italic mt-1">{consignmentSales.length} consignment item{consignmentSales.length > 1 ? 's' : ''} not included in cash total.</p>
                    )}
                  </div>
                ) : (
                  /* Pure consignment — show M-Pesa method only, no amount */
                  <div className="pb-4 border-b border-dashed border-gray-200 text-sm">
                    <h3 className="font-semibold text-lg mb-2 text-gray-800">Payment Details</h3>
                    <div className="flex items-center justify-between py-2 px-3 bg-purple-50 rounded-lg border border-purple-100">
                      <span className="font-semibold text-purple-700">M-Pesa</span>
                      <span className="text-xs text-gray-400 italic">Financed — amount confidential</span>
                    </div>
                    {paymentData[0]?.transactionId && (
                      <div className="flex justify-between py-1 text-sm text-gray-600 mt-1">
                        <span>Transaction ID:</span>
                        <span className="font-mono">{paymentData[0].transactionId}</span>
                      </div>
                    )}
                    <p className="text-xs text-purple-500 italic mt-2">
                      This is a financed consignment sale. Amount details are not disclosed on customer receipt.
                    </p>
                  </div>
                )}
          ...
          {/* Action Buttons */}
          <div className="no-print p-4 border-t flex flex-col sm:flex-row justify-around bg-gray-50 rounded-b-lg gap-2 sm:gap-4">
            <button
              onClick={handlePrint}
              className="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition-colors shadow-md w-full sm:w-auto min-h-[48px]"
            >
              Print Receipt
            </button>
            <button
              onClick={onClose}
              className="bg-gray-600 text-white px-6 py-2 rounded-lg hover:bg-gray-700 transition-colors shadow-md w-full sm:w-auto min-h-[48px]"
            >
              Close
            </button>
          </div>
